...
//--------------------------------------------會員登入API--------------------------------------------
// req (Request)：一個物件，裝著所有從前端傳來的資訊，包括你用 axios 送過來的帳號、密碼（會放在 req.body 裡面）。 
// res (Response)：一個物件，是我們要回傳給前端的「回覆單」。我們用它來呼叫 res.json() 把結果送回去。 
app.post('/api/login', async (req, res) => {
  ...
  if(result.length>0){
    //將資料庫取回的陣列資料，HTTP傳輸過程必須用JSON格式來傳回前端
    const user = result[0];
      res.json({
      id: user.id,
      name: user.username,
